...
impl PublicationTracker {
    ...
    pub(super) fn return_one(&mut self, token: BufferToken) -> Result<(), TrackerError> {
        ...
        self.slots[usize::from(token.bid)].state = SlotState::Returning;
        self.append_returning(token.bid, self.next_ticket);
        self.next_ticket = next_ticket;
        Ok(())
    }

    /// Append fresh publications for the whole bundle, or publish none of it.
    pub(super) fn return_bundle(&mut self, claim: &BundleClaim) -> Result<(), TrackerError> {
        ...
        // Returning is a transient reservation. It makes duplicate tokens in a
        // malformed range observable while still permitting complete,
        // allocation-free rollback during Drop.
        for (marked, token) in claim.iter().enumerate() {
            if let Some(error) = self.return_error(token) {
                for prior in claim.iter().take(marked) {
                    let slot = &mut self.slots[usize::from(prior.bid)];
                    debug_assert_eq!(slot.state, SlotState::Returning);
                    slot.state = SlotState::Owned;
                }
                return self.fail(error);
            }
        ...
        for (index, token) in claim.iter().enumerate() {
            self.append_returning(token.bid, start_ticket + index as u64);
        }
        self.next_ticket = next_ticket;
        Ok(())
    }
    ...
    fn append_returning(&mut self, bid: Bid, ticket: u64) {
...
impl fmt::Display for TrackerError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Poisoned => f.write_str("buffer publication tracker is poisoned"),
            Self::InvalidBid { bid, buf_count } => {
                write!(f, "buffer id {bid} is outside ring bounds ({buf_count})")
            }
            Self::InvalidBundleCount { count, buf_count } => write!(
                f,
                "bundle claims {count} buffers from a {buf_count}-buffer ring"
            ),
            Self::BidNotPublished { bid } => {
                write!(f, "buffer id {bid} has no live publication")
            }
            Self::BrokenLink { bid } => {
                write!(f, "publication links are inconsistent at buffer id {bid}")
            }
            Self::TicketGap {
                previous,
                next,
            ...
            Self::BundleExhausted { first, count } => write!(
                f,
                "bundle beginning at buffer id {first} has fewer than {count} live publications"
            ),
            Self::StaleReturn { bid, ticket } => write!(
                f,
                "buffer id {bid} return does not own publication ticket {ticket}"
            ),
            Self::WrongTracker {
                claim_buf_count,
                tracker_buf_count,
            ...
            Self::TicketOverflow => f.write_str("buffer publication ticket space exhausted"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn malformed_batch_rolls_back_every_transient_return() {
        let mut tracker = PublicationTracker::new(2);
