    ...
    fn poll_submit(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
        ...
        match this {
            StageProj::Unsubmitted { unsubmitted } => {
                let handle = match ready!(unsubmitted.poll(cx)) {
                    Ok(handle) => handle,
                    Err((handle, err)) => {
...
where
    T: 'static,
{
    type Output = Result<TypedHandle<T>, (TypedHandle<T>, io::Error)>;

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let this = self.project();
        match ready!(this.future.poll(cx)) {
            Ok(()) => Poll::Ready(Ok(this.handle.take().unwrap())),
            Err(err) => Poll::Ready(Err((this.handle.take().unwrap(), err.into()))),
        }
    }
}
